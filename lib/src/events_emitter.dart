...
class EventsEmitter<T> extends EventsListenable<T> {
  ...
  void emit(T event) {
    ...
    if (isDisposed) {
      Logger.root.warning('failed to emit event $event on a disposed emitter');
      return;
    }
    // queue mode
    if (_queueMode) {
  ...
  void updateQueueMode(bool newValue, {bool shouldEmitQueued = true}) {
    // check if already disposed
    if (isDisposed) {
      Logger.root.warning('failed to update queueMode on a disposed emitter');
      return;
    }
...
abstract class EventsListenable<T> extends Disposable {
  ...
  Future<void> cancelAll() async {
    if (_listeners.isNotEmpty) {
      ...
      Logger.root
          .finer('$objectId cancelling ${_listeners.length} listeners(s)');
  ...
  CancelListenFunc listen(FutureOr<void> Function(T event) onEvent) {
    //
    FutureOr<void> Function(T event) func = onEvent;
    if (synchronized) {
    ...
    cancelFunc() async {
      ...
      Logger.root.fine('$objectId event was cancelled by func');
    }
  ...
  CancelListenFunc on<E extends T>(
